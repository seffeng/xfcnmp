...
function fun_ins_php(){
    ...
    ##依次命令
    if [ "${php_pack_folder}" \> "php-7.2" ]; then 
    withLibzip=" --with-libzip=${libzip_ins_prefix}";
    else 
    withLibzip="";
    fi
    php_shl=(
        ...
        "if [ ! -f "${php_ins_prefix}/etc/php-fpm.d/www.conf" ] && [ -f ${php_ins_prefix}/etc/php-fpm.d/www.conf.default ]; then (mv ${php_ins_prefix}/etc/php-fpm.d/www.conf.default ${php_ins_prefix}/etc/php-fpm.d/www.conf) fi"
